...
class Content extends Component {
    ...
    handlePaginationChange = async (e, { activePage }) => {
        const { dataSubdomain } = await this.loadTableData(this.props.router.query.filename, activePage)
        this.setState(state => {
    ...
    async componentWillReceiveProps(nextProps) {
        if (this.props.dataDomain.subdomain !== nextProps.dataDomain.subdomain) {
            ...
            const { dataSubdomain } = await this.loadTableData(nextProps.router.query.filename, 1)
            this.setState(state => {
                state.dataSubdomain = dataSubdomain
                state.allPage = allPage
                state.activePage = 1
    ...
    async loadTableData(name, page) {
        const response = await fetch(`/api/search/score/file/${name}/url/${page}`)
    ...
    render() {
        const { router, dataDomain } = this.props
                    ...
                    <Grid columns='equal' textAlign='center' divided className="chartgrid">
                        <Grid.Row>
                            <Grid.Column style={{ minWidth: "247.31px" }}>
                                <Segment>
                                    <Statistic size='huge'>
                                        <Statistic.Label>Filename</Statistic.Label>
                                        <Statistic.Value>{router.query.filename}</Statistic.Value>
                                    </Statistic>
                                </Segment>
                            </Grid.Column>
                            <Grid.Column width={5}>
